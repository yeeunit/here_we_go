...
제 1 조 (목적)
이 약관은 여기있댕 (이하 "회사")가 제공하는 서비스의 이용과 관련하여 회사와 회원과의 권리, 의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.

제 2 조 (정의)
이 약관에서 사용하는 용어의 정의는 다음과 같습니다.

①"서비스"라 함은 구현되는 단말기(PC, TV, 휴대형단말기 등의 각종 유무선 장치를 포함)와 상관없이 "회원"이 이용할 수 있는 모캔버스 및 모캔버스 관련 제반 서비스를 의미합니다.
...
- 게시중단요청서비스 : mocanvas@huerey.com 이메일로 게시글 중단을 요청
제 17 조 (권리의 귀속)
...
1."회사"의 서비스를 이용함에 있어서 브라우저의 업데이트, 입점된 몰의 업데이트와 같은 외부 요인으로 인해서 발생하는 이용 장애에 대해서는 책임지지 않습니다.
2."회사"의 서비스를 타 사이트(블로그, 오픈마켓 등)에 등록하거나 플랫폼(카페24, 고도몰 등)에 등록하여 사용할 경우 각 타사의 컨텐츠 정책이 변경되어 발생하는 이용장애에 대해서는 책임지지 않습니다.
예시 ) style 태그나 css import, 혹은 html 태그에 제약을 두어 "회사"의 컨텐츠가 올바르게 등록되지 않는 모든 경우
예시 ) Attribute filter 및 html에 사용되는 일반적인 태그 양식들을 허용하지 않는 경우 ( Not Allowed Attribute Filtered )
3."회사"의 서비스는 기본적으로 HTML5를 지원하는 아래의 브라우저에서 사용이 가능하며, 구형 브라우저에 대한 별도의 지원을 하지 않습니다.
*데스크탑
Chrome 30+(자동 업데이트 포함)
Firefox 27+(자동 업데이트 포함)
...
부칙
①이 약관은 2020년 1월 1일부터 적용됩니다.
②2020년 8월 1일부터 시행되던 종전의 모캔버스 및 그와 관련된 서비스는 이 약관과 별개로 인정합니다.
(2020년 12월 31일 - 2021년 12월 31일 적용)
		</div>
	</body>
</html>